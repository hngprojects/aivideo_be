...
CHANGE_PWD_ENDPOINT = "/api/v1/auth/reset-password"
...
def test_autheniticated_user(mock_db_session, mock_user_service):
    ...
    user_pwd_change = client.post(
        CHANGE_PWD_ENDPOINT,
        json={"new_password": "Testpassword@123",
              "confirm_password": "Ojobonandom@123"},
    )
...
def test_wrong_pwd(mock_db_session, mock_user_service):
    ...
    assert user_pwd_change.json()["message"] == "Passwords do not match"
...
def test_user_password(mock_db_session, mock_user_service):
    ...
    print(user_pwd_change.json())
    assert user_pwd_change.status_code == 200
    assert user_pwd_change.json()["message"] == "Password has been reset successfully"
