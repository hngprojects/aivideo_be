...
class NotificationSettingService(Service):
    ...
    def update(self, db: Session, user_id: str, schema: NotificationSettingsBase):
        '''Updates or creates a notification service'''
        
        notification_setting = self.fetch_by_user_id(db=db, user_id=user_id)
        
        if notification_setting is None:
            # Create a new notification setting if it doesn't exist
            notification_setting = NotificationSetting(user_id=user_id)
