...
async def text_to_video_generate(request_text: TextInput,
    ...
    """
    response = await text_to_video_service.create(request_text, db)
...
async def text_to_video_update(request_text: VideoPatchRequest,
    ...
    Update a video.
        ...
        Raises:
            HTTPException: If anything goes wrong
    """
    response = await text_to_video_service.update(request_text, db)
   
    return response

@text_to_videos.get('/status', status_code=status.HTTP_200_OK,
                   response_model=VideoStatusResponse)
async def get_video_status(db: Annotated[Session, Depends(get_db)],
    ...
    Check the status of a video generation task.
        ...
        Raises:
            HTTPException: If anything goes wrong
    """
    response = await text_to_video_service.fetch(task_id, db)
    return response
