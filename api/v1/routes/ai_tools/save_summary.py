...
from api.v1.schemas.save_summary import SummaryRequest
...
async def save_summary_as_pdf(project_id: str, summary_request: SummaryRequest, db: Session = Depends(get_db)):
    # Validate project ID
    ...
    if not summary_request.summary.strip():
        raise HTTPException(status_code=400, detail="Summary string cannot be empty")
    
    # Generate PDF
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", size=12)
    pdf.multi_cell(0, 10, summary_request.summary)
